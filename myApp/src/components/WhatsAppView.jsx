...
function WhatsAppView({ token, onLogout, isActive }) {
    ...
    const checkConnectionStatus = async () => {
        ...
        try {
            ...
            // Use fallback isAuthenticating which checks RAM directly
            if (data.success && (data.data.connected || data.data.isAuthenticating)) {
                setIsConnected(true);
    ...
    if (!isConnected) {
        return (
            <IonPage>
                <IonContent className="ion-padding">
                    <QRScanner token={token} onConnected={handleConnected} onLogout={onLogout} />
                </IonContent>
            </IonPage>
        );
    }
